...
public class CustomViewAbove extends ViewGroup {
	...
	@Override
	public boolean onInterceptTouchEvent(MotionEvent ev) {//从上到下如果返回true,事件就会就地消费
		...
		switch (action) {
			...
			case MotionEvent.ACTION_DOWN:
				...
				mQuickReturn = false;
				int index = MotionEventCompat.getActionIndex(ev);//MotionEventCompat.getActionIndex(ev);
				mActivePointerId = MotionEventCompat.getPointerId(ev, index);//MotionEventCompat.getPointerId(ev,index);
				if (mActivePointerId == INVALID_POINTER)
					break;
				mLastMotionX = mInitialMotionX = MotionEventCompat.getX(ev, index); //MotionEventCompat.getX (ev,index);
				mLastMotionY = MotionEventCompat.getY(ev, index);//MotionEventCompat.getY (ev,index);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		final int action = ev.getAction();//action = ACTION.UP = ev.getAction();
	...
	@Override
	public void scrollTo(int x, int y) {//scrollTo (int x, int y);
		super.scrollTo(x, y);//scrollTo(int x, int y)意思就是将view移动到（x,y）坐标的位置上去
		mScrollX = x;
